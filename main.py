...
class mainapp(QWidget):
    ...
    def exex(self):
        ...
        if self.outfil.text() == "":
            ...
            showWarning(
                "NO OUTPUT FOLDER SPECIFIED", "Please specify an output folder."
            )
            return -1
        ...
        for i in range(len(inplocs)):
            ...
            for j in range(len(reflocs)):
                ...
                refcoord1 = [
                    refgt[0] + refgt[1] * ref_xsize,
                    refgt[3] + refgt[5] * ref_ysize,
                ]

                print(refcoord0, refcoord1)
                if (
                    inputcoord[0] > min(refcoord0[0], refcoord1[0])
            ...
            if refindex == 255:
                ...
                showWarning(
                    "NO MATCHING REFERENCE FILE FOUND for " + inputimgloc,
                    "Operation aborted for the file.",
                )
                continue
    ...
    def getinpfile(self):
        fname = QFileDialog.getOpenFileName(
            self, "Open file", "c:\\", "Image files (*.tif *.img)"
        )
        global inputimgloc
...
def run(inputimgloc, refimgloc, showMatches, rescalefac, band):
    ...
    inputimg = cv2.resize(
        inputimg, (int(heightinp / rescalefac), int(widthinp / rescalefac))
    )
    refimg = cv2.resize(
        refimg, (int(heightref / rescalefac), int(widthref / rescalefac))
    )
    ...
    for i in range(len(ip) - 1):
        for j in range(i + 1, len(ip)):
            distratio = (distance(ip[i][0], ip[i][1], ip[j][0], ip[j][1])) / (
                distance(ref[i][0], ref[i][1], ref[j][0], ref[j][1])
            )
            # print(ip[i], ref[i], ip[j], ref[j], distratio)
            if distratio > min(refbyinp_x, refbyinp_y) and distratio < max(
                refbyinp_y, refbyinp_x
            ):
                # print("YES")
                hash_list[i] += 1
                hash_list[j] += 1
    ...
    for i in range(len(good1_1)):
        ...
        if (
            inX - pad_inp_x < 0
            or inY - pad_inp_y < 0
            or inX + pad_inp_x >= len(inputimg)
            or inY + pad_inp_y >= len(inputimg[0])
        ):
            continue
        if (
            detX - padding < 0
            or detY - padding < 0
            or detX + padding >= len(refimg)
            or detY + padding >= len(refimg[0])
        ):
            continue

        # CORRELATION LAYER
        template2 = refimg[
            detY - padding : detY + padding, detX - padding : detX + padding
        ]
        template1 = inputimg[
            inY - pad_inp_y : inY + pad_inp_y, inX - pad_inp_x : inX + pad_inp_x
        ]
